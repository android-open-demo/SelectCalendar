...
import kotlinx.android.synthetic.main.sc_calendar_smart.view.calListView
import kotlinx.android.synthetic.main.sc_calendar_smart.view.headView
...
class CalendarView @JvmOverloads constructor(
    ...
    private var effect: Int = 2//效果，0只是显示，1单选，2范围选择
    private var afterEnable: Boolean = true//是否选今天之后的数据
    ...
    fun scrollToDate(date: String? = null) {
        ...
        val dateStr = when {
            d.length >= 6 -> d.substring(0, 4) + "年" + d.substring(4, 6) + "月"
            else -> def.format(Calendar.getInstance().time)
        }
    ...
    init {
        inflate(context, R.layout.sc_calendar_smart, this)
            ...
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                for (i in 0 until recyclerView.childCount) {
                    ...
                    if (1 == recyclerView.getChildViewHolder(childView).itemViewType) {
                        ...
                        showFirstMonthStr = mAdapter.data[index].monthStr
                    ...
                    2 -> {//多选
                        when (selData.size()) {
                            ...
                            1 -> {
                                ...
                                selectDataListener?.select(selData[0],selData[1])
                            }
                            2 -> {
    ...
    private fun parseAttr(context: Context, attrs: AttributeSet?) {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.SelectCalendar)
        order = typedArray.getInteger(R.styleable.SelectCalendar_sc_orader, 0)
        typedArray.recycle()
    }
    ...
    private fun productData(): List<CalendarItem> {
        ...
        for (y in startYear..endYear) {
            ...
            for (m in 0..11) {
                ...
                val monthStr = "${year}${String.format("%02d", month + 1)}"
    ...
    var selectDataListener: SelectDataListener? = null
    interface SelectDataListener{
        fun select(start:String,end:String)
    }
}